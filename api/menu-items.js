...
menuItemsRouter.post('/', validateMenuItem, (req, res, next) => {
  db.run(`INSERT INTO MenuItem (name, description, inventory, price, menu_id) VALUES ("${req.name}", "${req.description}", "${req.inventory}", "${req.price}", ${req.params.menuId})`,
    function(err, data) {
      if (err) {
        next(err);
      } else {
...
menuItemsRouter.put('/:menuItemId', validateMenuItem, (req, res, next) => {
  db.run(`UPDATE MenuItem SET name = "${req.name}", description = "${req.description}", inventory = "${req.inventory}", price = "${req.price}", menu_id = ${req.params.menuId} WHERE id = ${req.params.menuItemId}`,
    function(err, data) {
      if (err) {
        next(err);
      } else {
